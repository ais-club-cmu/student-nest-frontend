...
import type { UserProfileResponse, ListingDashboardResponse, ListingDashboardCard } from '@/lib/types/api.types';
        ...
        <div className="flex min-h-screen font-display bg-background-light dark:bg-slate-950/50">
            ...
            <aside className={`fixed top-16 bottom-0 left-0 z-30 w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col transition-transform duration-300 ease-in-out
                ...
                <nav className="flex-1 px-4 space-y-1 mt-4 overflow-y-auto">
                    <Link href="/student-portal" onClick={() => setSidebarOpen(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-primary/10 text-primary">
                        <span className="material-symbols-outlined text-[22px]">dashboard</span>
                        <span className="text-sm font-medium">Overview</span>
                    </Link>
                    <Link href="/student-portal/listings" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    ...
                    <Link href="/student-portal/notifications" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            ...
            <main className="flex-1 md:ml-64 p-4 md:p-8 relative min-w-0">
                <header className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">My Listing Portal</h2>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
                    {[
                        { label: 'Active', count: counts.active, icon: 'apartment', color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-900/30' },
                        { label: 'Under Review', count: counts.pending_review, icon: 'rate_review', color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-900/30' },
                        { label: 'Filled', count: counts.filled, icon: 'inventory_2', color: 'text-primary', bg: 'bg-blue-50 dark:bg-blue-900/30' },
                        { label: 'Drafts', count: counts.drafts, icon: 'edit_note', color: 'text-slate-500', bg: 'bg-slate-100 dark:bg-slate-800' },
                        ...
                        <div key={label} className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                            ...
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{dashboard ? count : '—'}</h3>
                        </div>
                    ))}
                </div>
                ...
                <div className="mb-10">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Link href="/student-portal/listings" className="text-primary text-sm font-bold flex items-center gap-1 hover:underline">
                            Manage All <span className="material-symbols-outlined text-sm">arrow_forward</span>
                        </Link>
                    </div>
                    {!ready ? (
                        <div className="flex items-center justify-center py-16 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
